...
EXPAND_FUNCTION_NAME = "expand_html"
"""
Name of the optional controller function that replaces an element with HTML
that may itself contain elements. See `docs/devElements.md`.
"""
...
def process(
    ...
    # Tags of the expanding elements whose expansions are currently being
    # processed, outermost first.
    expansion_stack: list[str] = []

    def process_element(
        element: lxml.html.HtmlElement,
    ) -> str | lxml.html.HtmlElement | SkipChildren | None:
        nonlocal result
        ...
        try:
            ...
            expand = get_module_function(mod, EXPAND_FUNCTION_NAME)

            # Check this in every phase so that authors find out when the
            # variant is created rather than when it is first rendered.
            if expand is not None and get_module_function(mod, "render") is not None:
                raise ValueError(
                    f"Element {element.tag} defines both render() and {EXPAND_FUNCTION_NAME}(). "
                    f"An element that defines {EXPAND_FUNCTION_NAME}() is rendered by rendering its expansion."
                )

            if method is None and expand is None:
                return None
            ...
            element_html = lxml.html.tostring(element)
            element_value = None

            if method is not None:
                args: list[Any] = [element_html, data]
            ...
            if expand is not None:
                if len(expansion_stack) >= MAX_EXPANSION_DEPTH:
                    chain = " > ".join([*expansion_stack, element.tag])
                    raise RecursionError(
                        f"Maximum element expansion depth ({MAX_EXPANSION_DEPTH}) exceeded: {chain}"
                    )

                # The expansion runs after this element's own phase function so
                # that it can depend on `params` stored by the element's `prepare()`.
                # Unlike phase functions, it must never change `data`: the same
                # expansion has to be reproducible in every phase.
                snapshot = copy.deepcopy(data)
                expansion = expand(element_html, data)
                if not isinstance(expansion, str):
                    raise TypeError(
                        f"Function {EXPAND_FUNCTION_NAME}() in {element_controller} must return a string"
                    )
                if data != snapshot:
                    raise ValueError(
                        f"Function {EXPAND_FUNCTION_NAME}() in {element_controller} must not modify data"
                    )
            ...
            if expansion is None:
                # TODO: validate that the render return value was a string?
                return element_value if phase == "render" else None

            # Process the expansion here rather than leaving it to the outer
            # traversal so that the depth limit and error notes work the same
            # way in every phase. In `render`, the outer traversal will re-parse
            # the returned string, but it no longer contains any known elements.
            expansion_stack.append(element.tag)
            try:
                if phase == "render":
                    return traverse_and_replace(expansion, replace_element)
                traverse_and_execute(expansion, execute_element)
            finally:
                expansion_stack.pop()

            # The expansion stands in for the element's original children, which
            # must therefore not be processed as well.
            ...
            if expansion_stack:
                exc.add_note(
                    f"This element was produced by expanding: {' > '.join(expansion_stack)}"
                )
            raise

    def replace_element(element: lxml.html.HtmlElement) -> ElementReplacement:
        value = process_element(element)
        # `SKIP_CHILDREN` is only ever returned outside of `render`.
        return None if isinstance(value, SkipChildren) else value

    def execute_element(element: lxml.html.HtmlElement) -> SkipChildren | None:
        value = process_element(element)
        return value if isinstance(value, SkipChildren) else None

    if phase == "render":
        result = traverse_and_replace(html, replace_element)
    else:
        traverse_and_execute(html, execute_element)
