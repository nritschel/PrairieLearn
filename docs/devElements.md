...
| `expand_html()` | `str` (html)  |                                                                                                          | all keys                                                                                                                                                                                                                                                                          | Returns HTML that replaces the element and may itself contain other elements. Called in every phase; cannot be combined with `render()`. See [Composable elements](#composable-elements).                                                                                                                                                                                        |
...
An element can be assembled from other elements by defining `expand_html()` instead of `render()`. It receives the same arguments as the other element functions and returns HTML that replaces the element:
...
def expand_html(element_html: str, data: pl.QuestionData) -> str:
    ...
    return f"""
        <p>Enter the real and imaginary parts:</p>
        <pl-number-input answers-name="{name}-re" label="$\\mathrm{{Re}}(z) =$"></pl-number-input>
        <pl-number-input answers-name="{name}-im" label="$\\mathrm{{Im}}(z) =$"></pl-number-input>
    """
```

The returned HTML is processed as if it had been written in `question.html` directly: every element it contains gets its own `prepare()`, `render()`, `parse()`, `grade()`, `test()`, and `file()` calls (and its own `expand_html()`, if it has one), and its client-side dependencies are loaded. Any children written inside the element in `question.html` are _not_ processed on their own; they are only available to `expand_html()` through `element_html`, which can include them in the expansion as needed.

`expand_html()` is called in every phase, so the same expansion has to be reproducible from the information that is available in all of them:

- It may depend on the element's attributes and children, `data["params"]`, `data["preferences"]`, the file path entries in `data["options"]`, and files on disk. It must not read phase-specific keys such as `data["panel"]`, `data["submitted_answers"]`, `data["partial_scores"]`, or the `*_url` entries in `data["options"]`, which are only present in some phases.
- In particular, the set of elements in the expansion and their `answers-name` attributes must be identical in every phase. Random values (e.g. from `pl.get_uuid()`) are fine for DOM `id` attributes but must never be used in an `answers-name`.
- It must not modify `data`. This is enforced.
- The returned HTML is not processed with Mustache and `<markdown>` blocks in it are not converted, since both of those happen before any element code runs.

An element that defines `expand_html()` may also define `prepare()`, `parse()`, `grade()`, `test()`, and `file()`; those run on the original element before the expansion is processed, so `expand_html()` can rely on `params` stored by the element's own `prepare()`. It must not define `render()`, since it is rendered by rendering its expansion. To combine the results of the expanded elements (for example, to give a single score for several inputs), use the question's `server.py` `grade()` function, which runs after every element's `grade()`.

Expansions can be nested up to a depth of 16.
